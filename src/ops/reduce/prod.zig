...
test "product reduction" {
    ...
    const A_row_product = try prod(testing.allocator, f32, A, &.{1});
    defer A_row_product.deinit();

    const A_col_product = try prod(testing.allocator, f32, A, &.{0});
    defer A_col_product.deinit();

    const A_product = try prod(testing.allocator, f32, A, &.{ 0, 1 });
